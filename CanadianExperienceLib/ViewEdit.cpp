...
ViewEdit::ViewEdit(wxFrame* parent) :wxScrolledCanvas(parent, wxID_ANY)
{
    ...
    parent->Bind(wxEVT_COMMAND_MENU_SELECTED, &ViewEdit::OnEditLeftMachineNum,
                 this, XRCID("EditMachineNumber(Left)"));
    parent->Bind(wxEVT_COMMAND_MENU_SELECTED, &ViewEdit::OnEditRightMachineNum,
                 this, XRCID("EditMachineNumber(Right)"));
    parent->Bind(wxEVT_COMMAND_MENU_SELECTED, &ViewEdit::OnSetMachineFrameLeft,
                 this, XRCID("SetLeftMachineStartFrame"));
    parent->Bind(wxEVT_COMMAND_MENU_SELECTED, &ViewEdit::OnSetMachineFrameRight,
                 this, XRCID("SetRightMachineStartFrame"));
}
...
{
    int frame = GetPicture()->GetTimeline()->GetCurrentFrame();
    double framerate = GetPicture()->GetTimeline()->GetFrameRate();
    GetPicture()->GetMachineAdapterLeft()->SetFrame(frame);
    GetPicture()->GetMachineAdapterLeft()->SetFrameRate(framerate);
    GetPicture()->GetMachineAdapterRight()->SetFrame(frame);
    GetPicture()->GetMachineAdapterRight()->SetFrameRate(framerate);
    Refresh();
}
...
/**
 * Handle OnEditLeftMachineNum menu option
 * @param event
 */
void ViewEdit::OnEditLeftMachineNum(wxCommandEvent &event) {

    GetPicture()->GetMachineAdapterLeft()->ShowDialogue(this);
    GetPicture()->UpdateObservers();
}
/**
 * Handle OnEditRightMachineNum menu option
 * @param event
 */
void ViewEdit::OnEditRightMachineNum(wxCommandEvent &event) {
    GetPicture()->GetMachineAdapterRight()->ShowDialogue(this);
    GetPicture()->UpdateObservers();
}
/**
 * Handle OnSetMachineFrameRight menu option
 * @param event
 */
void ViewEdit::OnSetMachineFrameRight(wxCommandEvent &event) {
    GetPicture()->GetMachineAdapterRight()->SetStartFrame(GetPicture()->GetTimeline()->GetCurrentFrame());
}
/**
 * Handle OnSetMachineFrameLeft menu option
 * @param event
 */
void ViewEdit::OnSetMachineFrameLeft(wxCommandEvent &event) {
    GetPicture()->GetMachineAdapterLeft()->SetStartFrame(GetPicture()->GetTimeline()->GetCurrentFrame());
}
